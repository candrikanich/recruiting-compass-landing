 ...
 *
 * Options:
 *   --project <name|id>   Vercel project (default: recruiting-compass-landing)
...
function parseArgs(argv) {
  const opts = {
    project: "recruiting-compass-landing",
  ...
  for (;;) {
    ...
    if (res.status === 429) {
      ...
      const resetMs = body?.error?.limit?.reset
        ...
        : 60_000;
      const waitMs = Math.max(resetMs, 5000) + 2000;
...
async function main() {
  ...
  for (const [branch, list] of byBranch) {
    ...
    for (const d of candidates) {
      if (aliasedIds.has(d.uid ?? d.id)) continue;
      if (cutoff != null && d.created > cutoff) continue;
